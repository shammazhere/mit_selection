    ...
    def upsert_risk_score(self, score_data: Dict[str, Any]):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """, (
                ...
                score_data.get("date") or datetime.now().strftime("%Y-%m-%d"),
                score_data.get("self_score") if score_data.get("self_score") is not None else 0.0,
                score_data.get("peer_score") if score_data.get("peer_score") is not None else 0.0,
                score_data.get("drift_score") if score_data.get("drift_score") is not None else 0.0,
                score_data.get("pre_multiplier_score") if score_data.get("pre_multiplier_score") is not None else 0.0,
                score_data.get("raw_fusion_score") if score_data.get("raw_fusion_score") is not None else 0.0,
                score_data.get("adjusted_fusion_score") if score_data.get("adjusted_fusion_score") is not None else 0.0,
                score_data.get("risk_score") if score_data.get("risk_score") is not None else 0.0,
                score_data.get("cohort_used") or "role",
                score_data.get("cohort_size") if score_data.get("cohort_size") is not None else 10,
                score_data.get("fallback_applied", 0),
                score_data.get("role") or "Unknown",
                score_data.get("name") or f"User {score_data.get('user_id', '')}",
                score_data.get("team", ""),
                score_data.get("cusum_path", "[]"),
                score_data.get("multipliers_applied", "{}"),
                score_data.get("score_components", "{}"),
                score_data.get("severity") or "low",
                score_data.get("explanation_text") or "",
